...
本轮没有因为 prose 重写新增新的知识线。M00 继续使用已经审查过的 APOSD complexity vocabulary 与 *Software Engineering at Google* 的 time/evolution perspective；M01 继续以 MIT 6.102 的 specification / AF-RI 训练为主要外部校准。
...
|---|---|---:|---:|
| M00 | lines | 660 | 287 |
| M00 | bytes | 16,007 | 19,929 |
| M00 | page-level H1 | 14 | 1 |
| M00 | H2 / H3 | 16 / 5 | 9 / 15 |
| M00 | `text` fences | 11 | 3 |
| M00 | `---` | 19 | 0 |
| M01 | lines | 950 | 392 |
| M01 | bytes | 18,270 | 22,849 |
| M01 | page-level H1 | 19 | 1 |
| M01 | H2 / H3 | 30 / 5 | 13 / 26 |
...
`naive flag → running job → cancellation request/state → concurrent transition → crash/restart → remote/retry/old client/migration`

直到“代码怎么写”已经不足以回答问题，才区分 programming problem 与 software engineering problem。

### baseline：软件为什么容易失控

第一版 rewrite 曾把这段压得过薄，直接从“新增 `CANCELLING` 要改很多文件”进入 change amplification。semantic review 后恢复了原文中很重要的因果：局部修改表面成本很低，因此 flag、copy、hardcode、adapter 等 individually reasonable decisions 可以长期累积，最后侵蚀 system model。没有恢复旧版的 slogan/fence 结构。

### baseline：change amplification / cognitive load / unknown unknowns

三者现在都由具体 pressure 引出：

- `CANCELLING` 的跨 surface 修改逼出 change amplification；
...
这些不再是四个平行定义 subsection。TaskForge candidate behavior table 先迫使作者回答 queued/running/terminal/missing，然后连续展开：

- running 不能直接伪装成 cancelled；
- success 到底对应哪个时间点；
...
M01 当前 behavior table **不是 M04 lab normative contract**。它故意采用 running cancellation request 与 already-cancelled success，用来展示 specification design space；M04 lab 后来采用更 strict 的 cancel semantics。

正文现在直接说明这点，并在 exercise 中要求学生不要照抄候选表，而要比较不同 contract strength / semantics。这样两个模块形成 transfer/comparison，而不是无意的跨章矛盾。

### baseline：representation / protocol / durable invariant

三类 invariant 全部保留。顺序从“单个 operation 的 behavior table 仍不足”自然进入“所有合法 state 必须共同满足什么”。

随后才引出 enforcement point，并用 `JobRepository.transition(...)` 作为候选 mechanism；同时明确它不是 repository pattern rule。最后自然过渡到 M02 的 state ownership / authority。
...
### baseline：tests from spec / illegal states / state machine / Agent workflow
...
- illegal states 先由 invariant pressure 引出，并保留 type-system limitation；
- lifecycle state machine 继续作为可 review protocol artifact；
...
`两个实现都 plausible → specification → responsibility boundary → pre/postcondition → partial function → behavior table → side effect/time/error/concurrency/repetition → idempotency pointer → invariant → enforcement point → state ownership pointer`
...
## 7. Cold-reader flow 自审

M00 当前主线是：

`cancel feature → 系统条件增加 → programming 不足 → complexity 怎样累积 → 三种 complexity symptoms → 四个工程对象 → modularity → Agent → 不要 over-design → review questions`

M01 接着同一个问题：

`实现能跑但 correctness 无法判断 → specification/contract → candidate behavior table → operation semantics → invariant → enforcement → specification strength/compatibility → test oracle → representation/state-machine tools → Agent workflow`
...
- M01 candidate cancel contract 与 M04 strict lab contract 的说明，对 cold reader 是否足够清楚；
- M01 从 operation semantics 到 invariant / enforcement 的 escalation 是否自然；
- M01 后半的 tests、illegal states、state machine、Agent workflow 是否仍属于同一条 argument，还是开始像附录式知识点回收；
